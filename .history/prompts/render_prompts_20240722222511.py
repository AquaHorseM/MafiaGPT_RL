...
def game_report(Game, player_id):
    """
    ...
    aliveness = [
        str(Game.all_players[x]) + " is " + ("ALIVE" if Game.all_players[x].is_alive else "DEAD")
        for x in (Game.alive_players + Game.dead_players)
    ]
    player = Game.all_players[player_id]
