...
class GameEnv:
    ...
    def init_env(self):
        ...
        for i in range(self.player_num):
            if self.all_players[i].role == "werewolf":
                self.action_space.append(Discrete(self.n_speak + self.n_vote + self.n_kill))
            elif self.all_players[i].role == "seer":
                self.action_space.append(Discrete(self.n_speak + self.n_vote + self.n_see))
            elif self.all_players[i].role == "medic":
                self.action_space.append(Discrete(self.n_speak + self.n_vote + self.n_heal))
            else: #villager
                self.action_space.append(Discrete(self.n_speak + self.n_vote))
    ...
    def discretify(self, actions, player_id):
        if isinstance(actions, list):
            return [self.discretify(action) for action in actions]
        assert isinstance(actions, str), "actions must be a string or a list of strings"
        if actions == "speak":
            return [1] * self.n_speak + [0] * (self.action_space_size[player_id] - self.n_speak)
    ...
    def get_available_actions(self, player_id):
        if self.game_status["cur_stage"] == "night":
            ...
            elif self.player_types[player_id] == "harl":
                return self.discretify(night_actions[self.all_players[player_id].get_role()])
