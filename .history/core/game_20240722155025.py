...
class Game:
        ...
        for i, num in enumerate(shuffled_nums):
            ...
            if role == "werewolf":
                if player_type == "baseline":
                    ...
                    self.alive_players[-1].special_actions_log.append(
                        f"you are werewolf and this is your team (they are all werewolf) : {werewolf_ids}"
                    )
                elif player_type == "reflex":
                    self.alive_players.append(WerewolfPlayer(i, init_global_info, init_werewolf_private_info, "core/players/prompts/werewolf"))
            elif role == "villager":
                if player_type == "baseline":
                    self.alive_players.append(Villager(role="villager", id=i))
            elif role == "medic":
                if player_type == "baseline":
                    self.alive_players.append(Medic(role="medic", id=i))
            elif role == "seer":
                if player_type == "baseline":
                    self.alive_players.append(Villager(role="seer", id=i))

            event = Event("set_player", {"id": i, "role": role, "player_type": player_type})
    ...
    def get_alive_werewolves(self):
        ls = list(filter(lambda player: player.get_role() == "werewolf", self.alive_players))
        # to make sure that last werewolf will make the last decision
        ls[-1].rank = "leader"
        return ls

    def get_alive_medics(self):
        return list(filter(lambda player: player.get_role() == "medic", self.alive_players))

    def get_alive_seers(self):
        return list(filter(lambda player: player.get_role() == "seer", self.alive_players))

    def get_alive_villagers(self):
        return list(
            filter(lambda player: player.get_role() == "villager", self.alive_players)
        )
    ...
    def kill(self, player_id):
        ...
        event = Event("kill", {"player": Player.id, "visible": "werewolf"})
        self.event_book.add_event(event)
        self.log_submit(event)
        return
