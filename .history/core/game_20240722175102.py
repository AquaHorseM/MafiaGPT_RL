...
class Game:
    ...
    def die(self, player_id):
        ...
        self.alive_players.remove(player_id)
        self.dead_players.append(player_id)
        self.add_event("die", {"player": player_id, "visible": "all"})
        return
    ...
    def run_day(self):
        ...
        for player_id in self.alive_players:
            ...
            if target_voted:
                ...
                self.report.append(f"{str(self.all_players[player_id])} Voted to {target_voted}")
            else:
                self.logger.warning(f"{str(self.all_players[player_id])} skipped the voting")
    ...
    def run_night(self):
        ...
        for seer_id in self.get_alive_seers():
            ...
            self.add_event({"event": "inquiry", "content": {"player": seer_id, "target": target, "is_werewolf": is_werewolf, "reason": reason}})
        ...
        for werewolf_id in werewolf_ids:
            ...
            if self.player_types[werewolf_id] == "baseline":
                ...
                self.all_players[werewolf_id].private_info["role_specific_info"]["previous_advices"] = advices
