...
class Game:
    ...
    def run_game(self):
        while True:
            ...
            if self.is_game_end():
                ...
                return
            #TEST
            self.save_checkpoint(f"checkpoints/game_{self.id}_{datatime.datetime.now().strftime('%Y%m%d%H%M%S')}")
            self.run_day()       
            if self.is_game_end():
                self.save_game_record()
                return
    ...
    def load_checkpoint(self, root_path): #assume all players are reflex players
        ...
        for i in range(info["global_info"]["player_num"]):
            player = load_player_from_checkpoint(os.path.join(root_path, f"player_{i}_ckpt.pkl"), self)
            player.event_book = self.event_book.filter(
