...
class GameEnv:
    ...
    def get_observation_single_player(self, player_id):
        if self.all_players[player_id].role == "werewolf":
            return {
                ...
                "role": 1,
                "companions": np.array([1 if i in self.all_players[player_id].get_companions() else 0 for i in range(self.player_num)]),
    ...
    def step(self, actions):
        ...
        if self.game_status["cur_stage"] == "night":
            ...
            self.game_status["start_speaking_player"] = None
