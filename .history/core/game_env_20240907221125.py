...
class GameEnv:
    ...
    def step(self, actions):
        ...
        if self.game_status["cur_stage"] == "night":
            ...
            self.game_status["speaking_player"] = None
            self.current_round["start_speaking_player"] = random.choice(self.alive_players)
            self.update_all_hstates()
        elif self.game_status["cur_stage"] == "day":
            speaking_player = self.game_status["speaking_player"]
            self.add_event({"event": "speak", "content": {"player": speaking_player, "speak_type": actions[speaking_player]}, "visible": speaking_player})
            self.game_status["cur_stage"] = "vote"
            self.game_status["start_speaking_player"] = speaking_player
