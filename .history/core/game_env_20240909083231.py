...
class GameEnv:
    ...
    def step(self, actions):
        ...
        if self.is_game_end():
            ...
            self.end()
        obs = [self.get_observation_single_player(i) for i in range(self.player_num)]
        dones = [1 if ((not self.all_players[i].is_alive) or self.game_status["winner"] is not None) else 0 for i in range(self.player_num)]
        state = self.get_state()
        return self._repeat(self.get_observation_single_player), self._repeat(self.get_state), rewards, dones, self.game_status, self.get_available_actions()
