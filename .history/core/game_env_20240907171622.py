...
class GameEnv:
    ...
    def get_observation_space(self, player_id):
        if self.all_players[player_id].role == "werewolf":
            return gym.spaces.Dict({
                "belief": gym.spaces.Box(low = 0, high = 1, shape = (self.player_num * self.player_num * 4,), dtype = np.float32),
                "role": gym.spaces.Discrete(1),
                "companions": gym.spaces.MultiBinary(self.player_num),
                "last_killed": gym.spaces.Discrete(1)
            })
    ...
    def init_env(self):
        ...
        self.shared_observation_space = gym.spaces.Dict({
            ...
            "dead_players": gym.spaces.MultiBinary(self.player_num),
