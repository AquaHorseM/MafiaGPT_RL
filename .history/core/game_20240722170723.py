        ...
        for werewolf_id in werewolf_ids:
            if self.player_types[werewolf_id] == "baseline":
                if self.all_players[werewolf_id].rank == "leader":
                    self.all_players[werewolf_id].killing(self)
                else:
                    self.all_players[werewolf_id].advicing(self)
                ...
                self.add_event({"event": "kill", "content": {"player": werewolf_id, "action": action, "target": target, "reason": reason, "visible": "werewolf"}})  
                for wid in werewolf_ids:
                    if wid != werewolf_id:
                        self.all_players[wid].hidden_state.set_advices(werewolf_id, reason)
        return
