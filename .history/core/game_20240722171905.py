...
class Game:
    ...
    def run_night(self):
        ...
        for medic_id in self.get_alive_medics():
            ...
            self.add_event({"event": "healing", "content": {"player": medic_id, "target": target, "reason": reason}})
            self.healed_guy = target
        for seer_id in self.get_alive_seers():
            ...
            self.add_event({"event": "inquiry", "content": {"player": seer.id, "target": target, "is_werewolf": is_werewolf, "reason": reason}})
        ...
        for werewolf_id in werewolf_ids:
            ...
            if self.player_types[werewolf_id] == "baseline":
                if self.all_players[werewolf_id].rank == "leader":
                ...
                self.private_info["role_specific_info"]["previous_advices"] = advices
                action, target, reason = self.all_players[werewolf_id]._act(self.event_book, available_actions = ["kill"])
                event_name = "kill" if werewolf_id == werewolf_ids[-1] else "advicing"
                self.add_event({"event": event_name, "content": {"player": werewolf_id, "action": action, "target": target, "reason": reason, "visible": "werewolf"}}) 
            ...
            if action == "kill":
                for wid in werewolf_ids:
                    if self.player_types[wid] == "baseline":
                        ...
                        self.previous_advices = advices
                    self.add_event({"event": "kill", "content": {"player": wid, "target": target, "visible": "werewolf"}})
        return
