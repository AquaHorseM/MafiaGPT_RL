        ...
        def __str__(self) -> str:
            ...
            for i in range(self.player_num):
                for j in range(self.player_num):
                    s += f"play {i} believes player {j} is role {np.argmax(self.beliefs[i][j])} with probability {np.max(self.beliefs[i][j])} \n"
            return s
                    
        def str_to_tensor(self, belief_str):
            #assume the str is in the format "play i believes player j is role k with probability p \n ..."
            def match(belief_str: str) -> (int, int, str, float):
                m = re.match(r"play (\d+) believes player (\d+) is role (\d+) with probability (\d+\.\d+)", belief_str)
