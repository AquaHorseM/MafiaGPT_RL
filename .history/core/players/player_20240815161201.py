    ...
    class HiddenState:
        ...
        def str_to_tensor(self, belief_str):
            ...
            def match(belief_str: str):
                ...
                #Remember the probability is a float, and the role should be a string
                pattern = r"player (\d+) believes player (\d+) is role () with probability (\d+\.?\d*)"
                match = re.match(pattern, belief_str)
                # print(f"match: {match}")
                if match is None:
                    return None
                return tuple(map(int, match.groups()[:-1])) + (float(match.groups()[-1]),)
