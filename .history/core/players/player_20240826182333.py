...
class Player:
    ...
    def reflex_single_pair(self, prev_hstate, next_hstate, pred_hstate):
    ...
    def reflex(self, data):
        ...
        for d in data:
            prev_hstate, events, next_hstate = d
            pred_hstate = self.hidden_state.beliefs[self.id]
            response = self.reflex_single_pair(prev_hstate, next_hstate, pred_hstate)
